...
GOOGLE_FORM_URL = "https://docs.google.com/forms/d/e/1FAIpQLSf62GcDTUDG9d23A5s5yVeNmHkGXtLrECPoPNXG5tl_vkp1XQ/viewform"

# ← map your JSON fields to the form’s entry IDs
FIELD_IDS = {
    "enumerator_name": "entry.111111",
    "patient_id":      "entry.222222",
    # questions & answers (up to 6)
    **{ f"q{i}": f"entry.1000{i}" for i in range(1,7) },
    **{ f"a{i}": f"entry.2000{i}" for i in range(1,7) },
    # diagnoses (up to 5)
    **{ f"d{i}_name":      f"entry.3000{i}" for i in range(1,6) },
    **{ f"d{i}_prob":      f"entry.4000{i}" for i in range(1,6) },
    **{ f"d{i}_reasoning": f"entry.5000{i}" for i in range(1,6) },
    **{ f"d{i}_sources":   f"entry.6000{i}" for i in range(1,6) },
    # summary
    "summary": "entry.700000",
}

def submit_to_google_form(parsed, enumerator_name, patient_id):
    form_data = {}

    form_data[FIELD_IDS["enumerator_name"]] = enumerator_name
    form_data[FIELD_IDS["patient_id"]]      = patient_id

    for idx, qa in enumerate(parsed.get("qa_pairs", []), start=1):
        form_data[FIELD_IDS[f"q{idx}"]] = qa["question"]
        form_data[FIELD_IDS[f"a{idx}"]] = qa["answer"]

    for idx, diag in enumerate(parsed.get("diagnoses", []), start=1):
        form_data[FIELD_IDS[f"d{idx}_name"]]      = diag["name"]
        form_data[FIELD_IDS[f"d{idx}_prob"]]      = diag["probability"]
        form_data[FIELD_IDS[f"d{idx}_reasoning"]] = diag["reasoning"]
        form_data[FIELD_IDS[f"d{idx}_sources"]]   = "; ".join(diag["sources"])
